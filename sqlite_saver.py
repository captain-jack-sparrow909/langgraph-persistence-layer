...
from langgraph.checkpoint.sqlite import SqliteSaver  # imported from langgraph-checkpoint-sqlite
import sqlite3
...
conn = sqlite3.connect("checkpoints.sqlite", check_same_thread=False)  #SQLite in Python raises an error if you try to use a connection from a different thread than the one that created it:
memory = SqliteSaver(conn=conn) # check_same_thread=False is a SQLite-specific setting that has nothing to do with LangGraph — it's about how Python's sqlite3 module handles threads.

graph = builder.compile(checkpointer=memory, interrupt_before=["human_in_the_loop"])
